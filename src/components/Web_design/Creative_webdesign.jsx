...
const Creative_webdesign = () => {
  ...
  return (
    <section className="relative overflow-hidden bg-white px-5 py-10 font-sans sm:px-8 lg:px-10 lg:py-12">
      ...
      <div className="pointer-events-none absolute right-0 bottom-0 h-72 w-72 rounded-full bg-blue-100/45 blur-3xl" />

      <div className="relative mx-auto max-w-[1160px]">
        <div className="text-center">
          <div className="inline-flex max-w-full flex-wrap items-center justify-center gap-2 rounded-full bg-blue-50 px-4 py-1.5 text-[11px] font-extrabold uppercase tracking-[0.13em] text-blue-800 sm:px-5 sm:text-xs">
            <SparklesIcon className="h-4 w-4" />
            Our Process
          </div>

          <h2 className="mx-auto mt-4 max-w-[900px] text-[28px] font-extrabold leading-tight tracking-normal text-[#07122f] sm:text-4xl lg:text-[46px]">
            A super creative web design process{" "}
            <span className="text-blue-800">that makes us different</span>
          </h2>
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {processSteps.map(({ number, title, text, icon: Icon, theme }, index) => (
            <article
              key={number}
              className={`relative min-h-[auto] overflow-hidden rounded-xl border border-slate-200 ${theme.border} border-b-4 bg-white p-5 shadow-[0_12px_34px_rgba(15,23,42,0.065)] sm:min-h-[235px]`}
        ...
        <div className="mt-8 flex flex-col items-center justify-center gap-3 text-center">
          <div className="hidden items-center gap-5 text-sm font-semibold leading-5 tracking-normal text-blue-800 sm:flex">
          ...
          <a
            href="/contact"
            className="inline-flex min-h-12 w-full items-center justify-center gap-4 rounded-full bg-gradient-to-br from-blue-800 to-blue-800 px-6 py-4 text-xs font-extrabold uppercase tracking-[0.06em] text-white shadow-[0_18px_42px_rgba(30,64,175,0.24)] transition hover:-translate-y-1 sm:h-12 sm:w-auto sm:min-w-[280px] sm:px-7 sm:py-0"
          >
            Design Your Website
